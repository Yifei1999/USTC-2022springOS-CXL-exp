...
void printCmdList()
{
    struct cmd* ptr = ourCmds;
    while (ptr != NULL)
    {
        myPrintf(0x7,"cmd name: %s\n",ptr->cmd);
        ptr = ptr->nextCmd;
    }
    myPrintf(0x7,"\n");
    return;
...
void addNewCmd(	unsigned char *cmdName, 
    int (*func)(int argc, unsigned char **argv), 
    ...
    */
    ...
    struct cmd* newPtr = (struct cmd*)malloc(cmd_size);
    
    newPtr->func = func;
    newPtr->help_func = help_func;
    strcpy(cmdName, newPtr->cmd);
    strcpy(description, newPtr->description);

    newPtr->nextCmd = ourCmds;
    ourCmds = newPtr;
    myPrintf(0x7,"addNewCmd: new commmand added\n\n");
